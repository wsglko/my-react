...
const BstList = () => {
    ...
    }).map((item, key) => <ul style={{ margin: 5 }} key={key} className="w3-ul w3-card-4"><li>BST Name: {item.bst_name}</li><li>BST ID: {item.bst_id}</li><li>City: {item.bst_city}</li><li>Owner: {item.provider}</li><li>Address: {item.bst_address}</li><li>Cooridnates: <a href={`https://www.google.com/maps/place/${item.latts},${item.longs}`}>{item.latts}, {item.longs}</a></li><li><Link to={`/ebm/${item.bst_id}/${item.bst_name}`}><button className="w3-btn w3-tiny w3-blue">EPM</button></Link></li></ul>)
    return (
        <div>
